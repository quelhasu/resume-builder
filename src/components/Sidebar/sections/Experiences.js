    ...
    <div>
      <Card title="Work Experiences">
        {experiences.map((item, index) => (
          <Form item={item} collapsed={true} modify={true} />
        ))}
        <Form />
      </Card>
    </div>
...
const Form = ({ collapsed = false, item = {}, modify = false }) => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    modify ? dispatch(modifyExp(exp)) : dispatch(createExp(exp))
  }
      ...
      <form onSubmit={handleSubmit}>
        ...
        {modify ? (
          ...
          <ButtonIcon type="submit" value="Apply" color="blue" px="4" />
            ...
            <ButtonIcon className="float-left" type="submit" value="Add" color="green" px="4" />
          )
        }
